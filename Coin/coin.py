...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'MinerX', amount = 10)
...
def add_transaction():
    ...
    if not all(key in json for key in transaction_keys):
        return 'Transaction elements missing', 400
...
@api.route('/connect_node', methods = ['POST'])
def connect_node():
    print('test')
    json = request.get_json()
...
@api.route('/replace_chain', methods = ['GET'])
def replace_chain():
